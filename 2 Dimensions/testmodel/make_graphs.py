...
trials = ['xx', 'yy', 'xy']
for trial in trials:
    df = pd.read_csv("../dataset/data_" + trial + ".csv")
    pred = pd.read_csv("results/predictions_" + trial + "_2d.csv")
    style = 'ggplot'
    ...
    save_graph(legend, 3, data, labels, 'graphs/' + trial + '/comp_dX_2d', 0)
    ...
    save_graph(legend, 3, data, labels, 'graphs/' + trial + '/comp_X_2d', 0)
    ...
    data = [df['Time'], pred['S11'], df['S11'], pred['S22'], df['S22'],
            pred['S12'], df['S12']]
    labels = ['Time [s]', 'Stress [MPa]']
    save_graph(legend, 3, data, labels, 'graphs/' + trial + '/comp_S_2d', 0)
    ...
    labels = ['Time [s]', 'Stress [MPa]']
    save_graph(legend, 2, data, labels, 'graphs/' + trial + '/comp_R_dR_2d', 0)
    ...
    save_graph(legend, 3, data, labels, 'graphs/' + trial + '/comp_ET_2d', 1)
    ...
    data = [df['Time'], pred['Ei11'], df['Ei11'], pred['Ei22'], df['Ei22'],
            pred['Ei12'], df['Ei12']]
    labels = ['Time [s]', 'Strain [%]']
    save_graph(legend, 3, data, labels, 'graphs/' + trial + '/comp_Ei_2d', 0)
    ...
    data = [df['Time'], pred['dEi11'], df['dEi11'], pred['dEi22'], df['dEi22'],
            pred['dEi12'], df['dEi12']]
    labels = ['Time [s]', 'Strain [%]']
    save_graph(legend, 3, data, labels, 'graphs/' + trial + '/comp_dEi_2d', 0)
    ...
    labels = ['Time [s]', 'Strain [%]']
    save_graph(legend, 2, data, labels, 'graphs/' + trial + '/comp_p_dp_2d', 0)
    # Duvidas :
