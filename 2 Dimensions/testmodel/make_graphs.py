...
for trial in trials:
    ...
    save_graph(legend, 3, data, labels, workd_gra + trial + '/comp_dX_' + trial
               + '_2d', 0)
    ...
    save_graph(legend, 3, data, labels, workd_gra + trial + '/comp_X_' + trial
               + '_2d', 0)
    ...
    data = [df['Time'], pred['S11'], df['S11'], pred['S22'], df['S22'],
            pred['S12'], df['S12']]
    labels = ['Time [s]', 'Stress [MPa]', trial]
    save_graph(legend, 3, data, labels, workd_gra + trial + '/comp_S_' + trial
               + '_2d', 0)
    ...
    labels = ['Time [s]', 'Stress [MPa]', trial]
    save_graph(legend, 2, data, labels, workd_gra + trial + '/comp_R_dR_' + trial
               + '_2d', 0)
    ...
    save_graph(legend, 3, data, labels, workd_gra + trial + '/comp_ET_' + trial
               + '_2d', 1)
    ...
    data = [df['Time'], pred['Ei11'], df['Ei11'], pred['Ei22'], df['Ei22'],
            pred['Ei12'], df['Ei12']]
    labels = ['Time [s]', 'Strain [%]', trial]
    save_graph(legend, 3, data, labels, workd_gra + trial + '/comp_Ei_' + trial
               + '_2d', 0)
    ...
    data = [df['Time'], pred['dEi11'], df['dEi11'], pred['dEi22'], df['dEi22'],
            pred['dEi12'], df['dEi12']]
    labels = ['Time [s]', 'Strain [%]', trial]
    save_graph(legend, 3, data, labels, workd_gra + trial + '/comp_dEi_' + trial
               + '_2d', 0)
    ...
    labels = ['Time [s]', 'Strain [%]', trial]
    save_graph(legend, 2, data, labels, workd_gra + trial + '/comp_p_dp_' + trial
               + '_2d', 0)
    # Close all figures
    plt.close('all')
