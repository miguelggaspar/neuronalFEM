...
C      DO K1 = 1, 1
      ...
      CLOSE(17)
      S11_temp = STRESS(1) + DDSDDE(1,1)*(DSTRAN(1)-dEi11)+
     +            DDSDDE(1,2)*(DSTRAN(2)-dEi22)
      S22_temp = STRESS(2) + DDSDDE(2,1)*(DSTRAN(1)-dEi11)+
     +            DDSDDE(2,2)*(DSTRAN(2)-dEi22)
      S12_temp = STRESS(3) + DDSDDE(3,3)*(DSTRAN(3)-dEi12)
C
C      Ei11 = Ei11 + dEi11*DTIME(1)
C      Ei22 = Ei22 + dEi22*DTIME(1)
C      Ei12 = Ei12 + dEi12*DTIME(1)
C      X11 = X11 + dX11*DTIME(1)
C      X22 = X22 + dX22*DTIME(1)
C      X12 = X12 + dX12*DTIME(1)
C      R = R + dR*DTIME(1)
C      p = p + dp*DTIME(1)
C
      a = save_state(Ei11, Ei22, Ei12, R, S11_temp, S22_temp,
...
C
      ...
      STRESS(2) = S22_temp
      STRESS(3) = S12_temp
C      STRAN(2) = STRAN(2) + DSTRAN(2)*DTIME(1)
C      WRITE(*,*)'TOTAL STRAIN -> ',STRAN(2)
C
...
C
      ...
      STRAN(1) = STRAN(1) + DSTRAN(1)*DTIME(1)
      STRAN(2) = STRAN(2) + DSTRAN(2)*DTIME(1)
      STRAN(3) = STRAN(3) + DSTRAN(3)*DTIME(1)
      open(unit=19, file='/home/miguel/state_hist.txt',
     +     position="Append", action='write')
      WRITE(19,*)Ei11,',',Ei22,',',Ei12,',',R,
     +           ',',S11,',',S22,',',S12,
     +            ',',X11,',',X22,',',X12,',',p,
     ...
     +            ',',KINC,',',KSTEP,',',NOEL,',',NPT,',',KSPT
      ...
      open(unit=19, file='/home/miguel/deriv_hist.txt',
     +     position="Append", action='write')
      WRITE(19,*)dEi11,',',dEi22,',',dEi12,',',dR,',',dX11,',',dX22
     +           ,',',dX12,',',dp
      CLOSE(19)
...
** PREDICT STATE DERIVATIVES GIVEN A CERTAIN STATE *
****************************************************
      real(8) function  save_state(Ei11, Ei22, Ei12, R, S11, S22,
     1 S12, X11, X22, X12, p, DSTRAN1, DSTRAN2, DSTRAN3,
     2 KINC, KSTEP, NOEL, NPT, KSPT)
c
      real(8) Ei11, Ei22, Ei12, R, X11, X22, X12, p, S11, S22, S12,
     + DSTRAN1, DSTRAN2, DSTRAN3, S11_temp
      integer KINC(1), KSTEP(1), NOEL, NPT, KSPT
c
      open(unit=16, file='/home/miguel/state_debug.txt')
      WRITE(16,*)S11,',',DSTRAN1,',',R,
     +            ',',X11,',',p,
     +            ',',KINC,',',NPT
      CLOSE(16)

      call system("/usr/bin/python /home/miguel/save_state.py")
      return
      end
*****************************************************************************
